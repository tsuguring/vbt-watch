    ...
    var body: some View {
        ScrollView {
            VStack {
                ...
                Text("\(objective.title)に適した重量はおよそ\(objective.weight)1RM(1回持ち上げることが限界の重さの\(objective.weight))である。1RMの何%であるかは挙上速度によって推測でき、あなたが最大の速度で上げた時の速度が\(String(objective.velocity))m/sである時の重量は\(objective.weight)1RMである。").font(.system(size: 14))
